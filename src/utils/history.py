...
class TranscriptionEntry:
    ...
    @classmethod
    def create(cls, text: str, duration: float, language: str, processing_time: float) -> TranscriptionEntry:
...
class TranscriptionHistory:
    ...
    def __init__(self, max_size: int = DEFAULT_MAX_SIZE, persist: bool = False) -> None:
    ...
    def add(self, text: str, processing_time: float = 0.0, duration: float = 0.0, language: str = "fr") -> None:
        ...
        entry = TranscriptionEntry.create(
            text=text.strip(), duration=duration, language=language, processing_time=processing_time
        )
        with self._lock:
    ...
    def add_from_result(self, text: str, duration: float, language: str, processing_time: float) -> None:
    ...
    def _save_unsafe(self) -> None:
        ...
        try:
            ...
            logger.warning("Erreur sauvegarde historique: %s", e)
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                logger.info("Historique charge: %d entrees", len(self._history))
        except Exception as e:
            logger.warning("Erreur chargement historique: %s", e)
...
def add_to_history(text: str, duration: float, language: str, processing_time: float) -> None:
