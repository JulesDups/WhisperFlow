...
class AudioEngine:
    ...
    def _audio_callback(self, indata: NDArray[np.float32], frames: int, time_info, status: sd.CallbackFlags) -> None:
        """
    ...
    def start_recording(self) -> bool:
        ...
        with self._recording_lock:
            ...
            try:
                ...
                logger.error("Erreur demarrage audio: %s", e)
                return False

    def stop_recording(self) -> AudioChunk | None:
        """
        ...
        return AudioChunk(data=audio_data, timestamp=self._start_time, sample_rate=self.sample_rate, duration=duration)
...
class AudioLevelMonitor:
    ...
    def update(self, level: float):
        ...
        with self._lock:
            # Lissage exponentiel
            self._current_level = self.smoothing * level + (1 - self.smoothing) * self._current_level
