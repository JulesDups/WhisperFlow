...
try:
    ...
    from transformers import AutoModelForSeq2SeqLM, AutoTokenizer, pipeline  # noqa: F401
...
class SmartFormatter:
    ...
    def __init__(self, level: FormattingLevel = FormattingLevel.SMART, model_id: str | None = None):
        self.level = level
    ...
    def load_model(self) -> bool:
        ...
        if not _HAS_TRANSFORMER:
            logger.warning("Transformers non disponible, formatage basique uniquement")
            return False
        ...
        try:
            ...
            logger.info("Modele de formatage charge: %s", self.model_id)
            return True

        except (OSError, RuntimeError) as e:
            logger.error("Erreur chargement modele formatage: %s", e)
            logger.info("Utilisation du formatage basique")
            return False
        finally:
            self._is_loading = False
    ...
    def format(self, text: str) -> FormattingResult:
        ...
        if not text or not text.strip():
            return FormattingResult(
                original_text=text, formatted_text=text, corrections_made=0, level_used=FormattingLevel.NONE
            )
        ...
        if self.level == FormattingLevel.NONE:
            return FormattingResult(
                original_text=original, formatted_text=original, corrections_made=0, level_used=FormattingLevel.NONE
            )
    ...
    def _format_smart(self, text: str) -> tuple[str, int]:
        ...
        try:
            ...
            corrections = sum(1 for a, b in zip(text.lower(), result.lower(), strict=False) if a != b)
            corrections += abs(len(text) - len(result))

            return result, corrections

        except (RuntimeError, ValueError) as e:
            logger.warning("Erreur formatage IA: %s, fallback basique", e)
            return self._format_basic(text)

    def _apply_punctuation_predictions(self, text: str, predictions: list[dict]) -> str:
        """
    ...
    def unload_model(self):
        ...
        logger.info("Modele de formatage decharge")
...
class RuleBasedFormatter:
    ...
    def format(cls, text: str) -> str:
        ...
        # Capitalise après .!?
        result = re.sub(r"([.!?])\s+([a-zàâäéèêëïîôùûüç])", lambda m: m.group(1) + " " + m.group(2).upper(), result)
        ...
        if result and result[-1] not in ".!?":
            ...
            if any(
                q in result.lower()
                for q in ["est-ce que", "qu'est-ce", "pourquoi", "comment", "quand", "où", "qui", "quel", "combien"]
            ):
                result += " ?"
            else:
                result += "."
