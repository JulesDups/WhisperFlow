...
from ..config import OutputMode, WindowMode, app_config, hotkey_config, ui_config
...
from .styles import get_main_stylesheet, get_state_colors
from .workers import AudioRecorderWorker, ModelLoaderWorker, TranscriptionWorker
...
class MainWindow(QMainWindow):
    ...
    def _setup_hotkeys(self):
        ...
        # Copier
        self.hotkey_listener.register(
            hotkey_config.COPY_TO_CLIPBOARD_KEY, on_press=self._copy_transcription, description="Copier"
        )

        # Quitter
        self.hotkey_listener.register(hotkey_config.QUIT_KEY, on_press=self.close, description="Quitter")
    ...
    def _auto_type_text(self, text: str):
        ...
        if type_text(text, use_clipboard=True):
            self.status_label.setText("⌨️ Texte inséré!")
        else:
            self.status_label.setText("⚠️ Impossible d'insérer le texte")
    ...
    def _copy_transcription(self):
        ...
        if self._last_transcription and copy_to_clipboard(self._last_transcription):
    ...
    def _change_ptt_key(self, new_key: str):
        ...
        self.hotkey_listener.register(
            new_key, on_press=self._on_ptt_press, on_release=self._on_ptt_release, description="Push-to-Talk"
        )
        ...
        logger.info("Touche PTT changee: %s -> %s", old_key.upper(), new_key.upper())
